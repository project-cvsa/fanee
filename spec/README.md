...
| **Message formatting mode** | A manifest field that controls how Runtime tools interpret Message values; See Section 6.4 |
...
Additionally, plugins and other tools **MAY** place their own directory structures at the Root Directory level for purposes beyond the core OTB specification. Tool-specific directories **MUST** begin with `x-` (e.g., `x-mytool/`, `x-plugin-name/`). These directories and their contents are outside the scope of this specification.
...
| `formatting` | String | **OPTIONAL** | Runtime | Controls how the Runtime interprets Message values. This specification defines three standard values: `"mf2"`, `"mf1"`, and `"identity"`. Tools **MUST** support `"mf2"` and `"identity"`. Support for `"mf1"` is **OPTIONAL**. Tools **MAY** use other values for custom formatting modes. Default: `"mf2"`. See Section 6.4. |
...
Message values **SHALL** be interpreted according to the `formatting` field specified in `manifest.json` (see Section 5). This specification defines the following standard formatting modes:

- **`"mf2"` (default):** Tools **MUST** support this mode. Message values **MUST** be valid [Unicode MessageFormat 2.0](https://www.unicode.org/reports/tr35/tr35-messageFormat.html) strings. If the `formatting` field is absent, tools **MUST** assume `"mf2"`.
- **`"identity"`:** Tools **MUST** support this mode. Message values are opaque strings. The Runtime **MUST NOT** perform any formatting, interpolation, or transformation on the value.
- **`"mf1"`:** Support for this mode is **OPTIONAL**. If a tool supports `"mf1"`, Message values **MUST** be valid [ICU MessageFormat 1](https://unicode-org.github.io/icu/userguide/format_parse/messages/) strings. Tools that do not support `"mf1"` **SHOULD** fall back to `"identity"` behavior and **MAY** emit a warning.

Tools **MAY** define and use other `formatting` values for custom formatting modes. If a Runtime tool encounters a `formatting` value it does not recognize, it **SHOULD** fall back to `"identity"` behavior (return the Message value as-is) and **MAY** emit a warning.
...
- A Runtime tool **SHOULD** provide a mechanism to set the Resolution context.
...
- A Runtime tool **MUST** support the `"mf2"` and `"identity"` formatting modes. Support for `"mf1"` is **OPTIONAL** (see Section 6.4). If `formatting` is absent, the tool **MUST** default to `"mf2"`. If the value is unrecognized or unsupported, the tool **SHOULD** fall back to `"identity"` behavior and **MAY** emit a warning.
...
Plugins and external tools **MAY** create directories at any level within the bundle for their own purposes. Tool-specific directories **MUST** begin with `x-` (e.g., `x-mytool/`, `x-plugin-name/`). Examples include but are not limited to:
